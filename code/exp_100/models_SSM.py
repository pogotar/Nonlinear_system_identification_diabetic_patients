...
from utils_SSM import set_params, ensure_3d, fun_start_controller, fun_start_controller_simple, plot_glucose_insulin, moving_average_online
...
def monotonic_gain_loss(y_hat, u, scaler_y, scaler_u, y, evaluate = 'meal', cumulative_window = 12*1.5, horizon=12*1.5, 
                        aggressive_coeff=0.7, CR = 1.0, CF = 1.0, G_bar = torch.tensor([110]), return_debug=False, type_preprocess_insulin = 'sum', loaded_parameters=None):
    """
    Enforces positive gain over a horizon for positive inputs (VECTORIZED).
    ...
    if type_preprocess_insulin == 'iob':
        u_sum = moving_average_online(u, window_size=loaded_parameters.Patient_par.iob_curve.shape[0], weights_mode='custom', custom_weights=torch.flip(loaded_parameters.Patient_par.iob_curve, dims=[0]))
        u_sum = u_sum[:, cumulative_window:-horizon+1, :]
        
    elif type_preprocess_insulin == 'sum':
...
def monotonic_gain_loss(y_hat, u, scaler_y, scaler_u, y, evaluate = 'meal', cumulative_window = 12*1.5, horizon=12*1.5, 
    ...
    if type_preprocess_insulin == 'sum':
        u_shifted = u.unfold(dimension = 1, size = cumulative_window, step = 1) # torch.Size([12, 565, 1, 12])  
        u_shifted = u_shifted.permute(0, 1, 3, 2) # torch.Size([12, 565, 12, 1])  
        ...
        # somma delle 
        u_sum = u_shifted.sum(dim=2)  # torch.Size([12, 529, 1])
    elif type_preprocess_insulin == 'iob':
        u_sum = moving_average_online(u, window_size=loaded_parameters.Patient_par.iob_curve.shape[0], weights_mode='custom', custom_weights=torch.flip(loaded_parameters.Patient_par.iob_curve, dims=[0]))
        u_sum = u_sum[:, cumulative_window:-horizon+1, :]
...
class ClosedLoopSystem_SSM(nn.Module):
    ...
    def run(self, u_ext, time, y_t0=None):
        """
        Simulates the closed-loop system for a given initial condition.
        ...
        # y0 = self.y0_from_x0(self.system_model.x)
        y = y_t0   # First noise realization
        ...
        for i, t in enumerate(time[0,:,0]):
            ...
            u_traj.append(torch.cat((u0_t[:,None,:], u1_t[:,None,:]), dim=-1))
    ...
    def __call__(self, u_ext, time, y_t0=None):
        """
        Args:
            x0 (torch.Tensor): Initial state. Shape = (batch_size, 1, state_dim)
            u_ext (torch.Tensor): External input signal. Shape = (batch_size, horizon, input_dim)
        ...
        Returns:
            torch.Tensor: Trajectories of outputs (batch_size, horizon, output_dim) [y0, ..., y_T-1]
        """
        return self.run(u_ext, time, y_t0=y_t0)
