...
class Parameter:
    ...
    def __init__(self, patient):
        ...
        if "code" in str(current_dir):
            self.root = current_dir.parent.parent  # Torna a ./
        else:
            self.root = current_dir  # Sei già alla radice
        self.Patient_par = self.Patient_par(patient, self)
    ...
    class Patient_par:
        def __init__(self, patient,parent):
            ...
            CRtv_data = sio.loadmat(f'{parent.root}/data/CRtv.mat')
            self.CR_values_not_norm = torch.from_numpy(CRtv_data['CRtv']['values'][0, patient - 1]).double()
            iAP_data = sio.loadmat(f'{parent.root}/data/train/sc_2days_identification/s#adult#' + str(int(patient)).zfill(3) + '.mat')
            self.CR = torch.from_numpy(iAP_data['iAP']['RCM_param'][0,0]['CRpatientForModel'][0,0][0]).double()
