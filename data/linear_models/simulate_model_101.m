...
for main_folder = main_folders
    ...
    for sottocartella = sottocartelle
        ...
        for k = dati_paziente
            ...
            disp(['patient: ' num2str(k)   '  |  FIT: ' num2str(FIT) '  |  MSE: ' num2str(MSE)  '  |  dataset: ' sottocartella{1}])
            ...
            if delta_M == 0
                delta_G_empirico = G_discreto(idx_i) - min(G_discreto);
                delta_G_modello = G_sim_discreto(idx_i) - min(G_sim_discreto);
                disp(['delat G empirico:  ' num2str(delta_G_empirico)])
                disp(['delat G modello:  ' num2str(delta_G_modello)])
                disp(['delat G formula:  ' num2str(CF * delta_I)])
                
            else
                delta_G_empirico = max(G_discreto) - G_discreto(idx_m);
                ...
                % iAP.RCM_param.CRpatientForModel
